...
public class StockUpUpService {
    ...
    public Map query(String holder_id) {
        ...
        if (StringUtils.isBlank(holder_id)) {
            ...
            List<Stock> holder_stocks = mongoTemplate.findAll(Stock.class)
                                                        .stream()
                                                        .filter(stock -> StringUtils.equals(stock.getHolder_id(),holder_id))
                                                        .collect(Collectors.toList());
     ...
     * 将个人持股情况整合成持仓情况
     */
    private HolderStock transStocks(Holder holder,List<Stock> holder_stocks) {
        List<Stock> stocks_0 = holder_stocks.stream()
        ...
        holderStock.setHolder_id(holder.getHolder_id());
        holderStock.setHolder_name(holder.getHolder_name());
